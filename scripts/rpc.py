...
Format: 'user:u1 secret:s1 muser:mu1 msecret:ms1,user:u2 secret:s2 muser:mu2 msecret:ms2'""", required=False)
    ...
    *** Mutually exclusive with --disable-chap ***""", action='store_true')
    ...
    p.add_argument('-p', '--fsdev-io-pool-size', help='Enable the writeback cache', type=int)
    p.add_argument('-c', '--fsdev-io-cache-size', help='Disable the writeback cache', type=int)
    ...
    def fsdev_aio_create(args):
        print(rpc.fsdev.fsdev_aio_create(args.client, name=args.name, root_path=args.root_path,
                                         xattr_enabled=args.xattr_enabled))
    ...
    p.add_argument('-x', '--xattr_enabled', help='Enable the extended attributes', choices=[0, 1], type=int, default=0)

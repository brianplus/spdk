...
int
spdk_fsdev_set_instance_opts(struct spdk_fsdev *fsdev, const struct spdk_fsdev_instance_opts *opts)
{
	struct spdk_fsdev_instance_opts module_opts;
	int res;
...
#define SET_FIELD(field) \
	if (offsetof(struct spdk_fsdev_instance_opts, field) + sizeof(module_opts.field) <= module_opts.opts_size) { \
...
int
spdk_fsdev_get_instance_opts(const struct spdk_fsdev *fsdev, struct spdk_fsdev_instance_opts *opts,
			     size_t opts_size)
{
	if (!opts) {
...
#define SET_FIELD(field) \
	if (offsetof(struct spdk_fsdev_instance_opts, field) + sizeof(opts->field) <= opts_size) { \
...
fsdev_io_complete(void *ctx)
{
	...
	assert(fsdev_io->internal.cb != NULL);
	assert(spdk_get_thread() == spdk_fsdev_io_get_thread(fsdev_io));
	fsdev_io->internal.cb(fsdev_io, fsdev_io->internal.caller_ctx);
}
